...
class Wert:
    ...
    def bedingte_anfragen(self, modell):
        ...
        if self.Zeile.befehl == "JGE":
            ...
            self.modell.pc += 1
        if self.zeile.befehl == "DIV":
            if self.get() == 0:
                raise Exception("Error: Durch null geteilt")
            else:
                self.modell.akku = round(int(modell.akku)/int(self.wert.get(modell)))
...
class Zeile:
    ...
    def ausführen(self, modell):
        if (self.befehl == "LDA"):
            ...
            modell.akku = int(self.wert.get(modell))*int(modell.akku)
        elif self.befehl == "DIV":
            self.Wert.bedingte_anfragen(modell)
##            modell.akku = int(modell.akku)/int(self.wert.get(modell))
